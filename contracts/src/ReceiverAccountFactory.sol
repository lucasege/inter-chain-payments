...
import "./ReceiverAccount.sol";

/**
 * A sample factory contract for ReceiverAccount
...
contract ReceiverAccountFactory {
     ...
     */
    function createAccount(address owner, uint256 salt) public returns (ReceiverAccount ret) {
        address addr = getAddress(owner, salt);
        uint256 codeSize = addr.code.length;
        if (codeSize > 0) {
            return ReceiverAccount(payable(addr));
        }
        ret = ReceiverAccount(
            payable(
